...
app.controller('infiniteScrollCrtl', function($scope, $log, $http, $q, $timeout){
    ...
    function readBatch() {
            var d = $q.defer();
            $http.get('partials/other/largeLoad.json').success(function(data){
                d.resolve(data);
            });
            return d.promise;
    }
...
app.controller('memOptimizerCrtl', function($scope, $log, $http, $q, $timeout){
...
angular.module('progress', ['ng']).directive('progressBar', function ($rootScope) {
    return {
        restrict: 'A',
        scope: true,
        template: '<div>ROWS RENDERED: {{count}} of {{len}} || IT IS THE ({{percent}}%) OF TOTAL || THE SCROLL IS AT {{scrollPercent}}%</div>',
